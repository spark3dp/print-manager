...
function generatePrintable( data, next )
{
    frisby.create('prepare tray')
    .post( config.URL + '/print/trays/generatePrintable', {
        "id" : [ data.result.id ] } )
    .expectStatus( 202 )
    .afterJSON( function(json) {
        data.task_id = json.id;
...
function generatePrintableShouldFail( data, next )
{
    frisby.create('prepare tray')
    .post( config.URL + '/print/trays/generatePrintable', {
        "id" : [ data.result.id ] } )
    .expectStatus( 403 )
    //.inspectBody()
    .expectBodyContains("Tray not ready")
    .toss();
}


function validatePrintable( data, next )
{
...
// Create a nice tray with a torus and generate a printable file:
harness.runTests( [
